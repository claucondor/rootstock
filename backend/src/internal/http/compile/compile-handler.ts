...
export async function compileHandler(
  ...
  if (!body || typeof body.source !== 'string' || !body.source.trim()) {
    reply
      .code(400)
      .send({
        error: "El campo 'source' es requerido y debe ser un string no vacío.",
      });
    return;
  }
  ...
  try {
    const result = await compiler.compileSolidity(
      body.source,
      body.contractName
    );
    ...
    if (result.errors && result.errors.length > 0) {
      reply
        .code(400)
        .send({ errors: result.errors, warnings: result.warnings });
      return;
    }
    ...
    reply
      .code(500)
      .send({ error: err.message || 'Error interno de compilación' });
  }
}
