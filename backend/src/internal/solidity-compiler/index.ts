...
export class SolidityCompilerService {
  ...
  public async compileSolidity(
    ...
    try {
      ...
      const flattenedSource = await flattenSolidity(
        source,
        contractPath,
        originalPath
      );
      logger.info(
        { flattenedLength: flattenedSource.length },
        'Source code flattened'
      );

      // Use the provided contract name or extract it from the source code
      const extractedContractName =
        contractName || extractContractName(flattenedSource);
      logger.info(
